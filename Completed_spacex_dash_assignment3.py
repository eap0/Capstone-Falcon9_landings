...
def get_pie_chart(entered_site):
    #create a sub dataframe with only two columns
    ...
    #create new dataframe of site-specific rows for the selected site 
        onesite_df = pd.DataFrame()
        for index,row in filtered_df.iterrows():
            if row['Launch Site'] == entered_site:
                onesite_df = onesite_df.append(row)
        fig = px.pie(data_frame=onesite_df,
                     names='class', 
                     color= 'class',
...
def get_scatter_plot(entered_site, payload_bounds):
    ...
    #create new dataframe of site-specific rows for the selected site   
        ...
        for index,row in filtered_df.iterrows():
            if row['Launch Site'] == entered_site:
                onesite_df = onesite_df.append(row)
        fig1 = px.scatter(onesite_df, x='Payload Mass (kg)', y='class', color='Booster Version Category')
        return fig1
# Run the app
